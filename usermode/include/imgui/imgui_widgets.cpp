...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
	...
	window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, c::anim::active, 30.f);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? c::anim::active : hovered ? c::text::label::hovered : c::text::label::ddefault, g.IO.DeltaTime * 6.f);
	it_anim->second.shadow_thinkess = ImLerp(it_anim->second.shadow_thinkess, IsItemActive() ? 2.f : 0.5f, g.IO.DeltaTime * 6.f);

	
	if (!is_multiline)
	{
		GetWindowDrawList()->AddShadowRect(frame_bb.Min, frame_bb.Max, ImColor(0.f, 0.f, 0.f, it_anim->second.shadow_thinkess / 1.5f), it_anim->second.shadow_thinkess * 35.f, ImVec2(0,0), 0, c::elements::rounding);
		GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, c::anim::ddefault, c::elements::rounding);
	}
	...
	if (render_cursor || render_selection)
	{
			...
			for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
			{
				...
				else
				{
					...
					if (rect.Overlaps(clip_rect))
						window->DrawList->AddRectFilled(rect.Min, rect.Max, GetColorU32(c::accent, 0.2f), c::elements::rounding);
				}
		...
		{
	ImU32 col = is_displaying_hint ? c::label::ddefault: c::label::active;
	...
	it_anim->second.label = ImLerp(it_anim->second.label, CalcTextSize(buf_display).x < 1.f ? c::text::label::ddefault: utils::GetColorWithAlpha(c::text::label::ddefault, 0.f), g.IO.DeltaTime * 6.f);
